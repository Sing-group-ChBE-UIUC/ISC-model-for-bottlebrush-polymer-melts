                      ...
                      double grid_shift[3], double **coords, int *particle_type, double ****density_grids)
{
    ...
    if (pm_type == 1)
    {
        ...
        for (int i = 0; i < natom_total; ++i)
        {
            ...
            density_grids[atom_type][xneigh][ygrid][zgrid] += (1-tx) * ty * tz / rho_norm;
            density_grids[atom_type][xgrid][yneigh][zgrid] += tx * (1-ty) * tz / rho_norm;
            density_grids[atom_type][xgrid][ygrid][zneigh] += tx * ty * (1-tz) / rho_norm;
            density_grids[atom_type][xneigh][yneigh][zgrid] += (1-tx) * (1-ty) * tz / rho_norm;
            density_grids[atom_type][xneigh][ygrid][zneigh] += (1-tx) * ty * (1-tz) / rho_norm;
            density_grids[atom_type][xgrid][yneigh][zneigh] += tx * (1-ty) * (1-tz) / rho_norm;
            density_grids[atom_type][xneigh][yneigh][zneigh] += (1-tx) * (1-tx) * (1-tz) / rho_norm;
        }
    }
    // For debug:
    double local_des = 0;
