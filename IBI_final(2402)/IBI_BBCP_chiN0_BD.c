...
int main (int argc, const char * argv[])
{
	...
	*idum = initRan();
	...
	// IBI by comparing gtarget AB
	for(i=0; i<Nbin; i++)
	{
		if(gro_AA[i]>0 && gri_AA[i]>0 && !isinf(gri_AB[i]))
		{
			pmfi_1[i] = pmfo_ab[i]-alpha*log((double)gro_AA[i]/gri_AA[i]);
