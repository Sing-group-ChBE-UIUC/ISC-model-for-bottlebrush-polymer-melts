...
for(i = 0; i<nchain; ++i)
{
	for (j = 0; j<Nisc-1; j++)
	{
		if(type[i*Nisc+j]==2 && type[i*Nisc+j+1]==2)
		{
			rcut = 5.0;
		}
		else
...
for(i=0; i<nchain; i++)
{
	for(j=0; j<(Nisc-2); j++)
	{
		if(type[i*Nisc+j]==2 && type[i*Nisc+j+1]==2 && type[i*Nisc+j+2]==2)
		{
			kappa_bend = 1.053;
		}
		else
	...
	for(j = i+1; j<Ntot; j++)
	{
		...
		if(type[i] ==1 && type[j]==1)
		{
			...
			if(r<rp[dmax-1])// When r is within the table
			{
				...
				else if(r > rp[0] && r < Lx)//if r is not in table, linear interpolation
				{
					...
					{
						coeff = -(mfo[high_index]-mfo[low_index])/(high_index-low_index);					
					}
					U += pmfo[low_index] + (r-rp[low_index])*(pmfo[high_index]-pmfo[low_index])/(double)(rp[high_index]-rp[low_index]);
					Unb += pmfo[low_index] + (r-rp[low_index])*(pmfo[high_index]-pmfo[low_index])/(double)(rp[high_index]-rp[low_index]);
				}
		...
		else if(type[i] != type[j]) 
		{
			...
			if(r<rp[dmax-1])// When r is within the table
			{
				...
				else if(r <= rp[0])// if r is smaller than minimum cutoff, use minimum value
				{
					coeff = mfi[0];
					U += pmfi[0];
					Unb += pmfi[0];
