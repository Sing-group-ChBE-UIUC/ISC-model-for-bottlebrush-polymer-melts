...
{
	//VARIABLES!
    int cfg, Nbb, Nsc, fbranch, Ntot, nchain, Nisc, Natmtype, Nbonds, Nbndtype, Nangles, Nangletype, index, molecule;
    int dint1, dint2, dint3, low_index, high_index, Ntot_gr, cutoff_bin;
    ...
    double *gr_BB = calloc(Nbin, sizeof(double));
    double *gr_AB = calloc(Nbin, sizeof(double));
    double *gri_AA = calloc(Nbin, sizeof(double));
    double *gri_BB = calloc(Nbin, sizeof(double));
    ...
    double *alpha_r = calloc(Nbin, sizeof(double));
	...
	*idum = initRan();
    ...
    //Read cfg file initial structure
    if(cfg ==1) 
	{
        for(i=0; i<nchain; ++i)
		{
			for(j=0; j<Nisc; ++j)
			{
				fscanf(cfg_ini, "%d %d %d %lf %lf %lf\n", &index, &molecule, &type[i*Nisc+j], &rx_ini[i*Nisc+j], &ry_ini[i*Nisc+j], &rz_ini[i*Nisc+j]);
                if(molecule != i+1)
                {
    ...
    //     }
    // }
    // printf("cutoff = %lf, cutoff_bin = %d\n", rp[cutoff_bin], cutoff_bin);
    // for(i=0; i<cutoff_bin; i++)
    // {
    //     alpha_r[i] = alpha;
    //     printf("%lf\n", alpha_r[i]);
    // }
    // for(i=cutoff_bin; i<Nbin; i++)
    // {
    //     alpha_r[i] = alpha*exp(-0.05*(i-cutoff_bin));
    //     printf("%lf\n", alpha_r[i]);
    // }
    ...
    for(iteration=0; iteration<maxiter; iteration++)
	{
        ...
        #include "Interactions_IBIBBCP.h"
			...
			//Every so often, the last 10% of timestep, calculate g(r)
			if(t > (tmax-tmax*0.9))
			{
                //Calculate AA/BB pair
                for(i = 0; i<Ntot; ++i)
                {
                    for(j = 0; j<Ntot; ++j)
                    {
                        if(i!=j && type[i]==1 && type[j]==1)//AA pair 
                        {   
                            if((int)i/Nisc == (int)j/Nisc && abs((int)i%Nisc - (int)j%Nisc) < 2)//In the same chain
                            {}
                            else
                            {
                                dx = rx[i]-rx[j];
                        ...
                        if(i!=j && type[i]==2 && type[j]==2)//BB pair 
                        {   
                            if((int)i/Nisc == (int)j/Nisc && abs((int)i%Nisc - (int)j%Nisc) < 2)//In the same chain
                            {}
                            else
		...
		for(i = 0; i<Nbin; ++i)
		{
            ...
            gri_BB[i] = (double) gr_BB[i]/norm;
            fprintf(gri_IBI_0, "%lf %lf %lf\n", ro_AA[i], gri_AA[i], gri_BB[i]);
		}
        fclose(gri_IBI_0); 
        //AB pair
        ...
        for(i = 0; i<Nbin; ++i)
        {
            ...
            shell_volume = 4*M_PI*r_low*r_low*binsize;
            number_density = Ntot_gr/(Lx*Ly*Lz);
            norm = (double) 2*Ntot_gr*shell_volume*number_density*(tmax*0.9-1);
        ...
        /// BI by comparing gtarget 
        for(i=0; i<Nbin; i++)
        {
            if(gri_AB[i]>0 && gro_AB[i]>0 && !isinf(gri_AB[i]))
            {
                pmfi_1[i] = pmfi[i]-alpha*log((double)gro_AB[i]/gri_AB[i]); //-0.1*(1-rp[i]/(rcut*3));
            }
            else
            {
                pmfi_1[i] = pmfi[i];
    ...
    free(rx_ini); free(ry_ini); free(rz_ini); free(RR);
    free(rp); free(pmfo); free(pmfo_ab); free(mfo); free(mfo_ab); free(ro_AA); free(ro_AB); free(gro_AA); free(gro_AB);
    free(gr_AA); free(gr_BB); free(gr_AB); free(gri_AA); free(gri_BB); free(gri_AB); free(pmfi); free(pmfi_1); free(mfi); free(mfi_1);
    free(idum);
    free(alpha_r);
    return 0;
